...
const URL_AUDIT_PATH = 'reports/url_status_audit_latest.json';
const TEMPLATE_OUT_DIR = 'data/fill_templates_v1_5';
const SOURCE_VERIFY_REPORT = 'reports/source_manual_verify_latest.json';
const TODAY = '2026-02-16';
const NOW_LOCAL = '2026-02-16T13:30:00+08:00';
const NEXT_REVIEW = '2026-06-30';
const PLACEHOLDER_URL = 'https://pending.example.com/fill-source-url';

const SOURCE_STATUS_HINT = {
  'stats.gov.cn': 200,
  'mohrss.gov.cn': 200,
  'chinajob.mohrss.gov.cn': 200,
  'ncss.cn': 200,
  'zhaopin.com': 200,
  'zhipin.com': 200,
  '51job.com': 200,
...
const SOURCE_TYPE_CANONICAL_MAP = {
  government: 'government_agency',
  platform: 'general_platform',
  mapped_topic: 'curated_mapping',
  registry_mapped: 'curated_mapping',
...
const FINANCE_INDUSTRIES = new Set([
  'IND_FIN_BANK',
  'IND_FIN_SECURITIES_FUND',
  'IND_FIN_INSURANCE',
]);

const PUBLIC_INDUSTRIES = new Set([
  'IND_CIVIL_SERVICE',
...
const SEED_SOURCES = [
  ...
  {
    source_id: 'SRC_MOE_MEETING_2026_JAN',
    source_name: '教育部-2026届高校毕业生就业创业工作会议（2026-01-07）',
    source_type: 'government_policy',
  ...
  {
    source_id: 'SRC_MOE_PROMOTION_2025_OCT',
    source_name: '教育部-部署做好2026届高校毕业生就业工作（2025-10-11）',
    source_type: 'government_policy',
    source_url: 'https://www.moe.gov.cn/jyb_xwfb/gzdt_gzdt/s5987/202510/t20251011_1416332.html',
    credibility: 'high',
    typical_update_cycle: 'event_driven',
    last_checked: TODAY,
  },
  {
    source_id: 'SRC_MOE_AUTUMN_2025',
    source_name: '教育部-开展2025年高校毕业生就业创业政策宣传月（2025-09-27）',
    source_type: 'government_policy',
    source_url: 'https://www.moe.gov.cn/jyb_xwfb/gzdt_gzdt/s5987/202509/t20250927_1415177.html',
    credibility: 'high',
    typical_update_cycle: 'event_driven',
    last_checked: TODAY,
  },
  {
    source_id: 'SRC_MOHRSS_CAMPUS_2026_0121',
    source_name: '人社部中国公共招聘网-高校毕业生联合招聘专场（2026-01-21）',
    source_type: 'government_platform',
    source_url: 'https://chinajob.mohrss.gov.cn/h5/c/2026-01-21/483434.shtml',
    credibility: 'high',
    typical_update_cycle: 'event_driven',
    last_checked: TODAY,
  },
  {
    source_id: 'SRC_MOHRSS_CAMPUS_2026_0129',
    source_name: '人社部中国公共招聘网-春季促就业活动（2026-01-29）',
    source_type: 'government_platform',
    source_url: 'https://chinajob.mohrss.gov.cn/h5/c/2026-01-29/486013.shtml',
    credibility: 'high',
    typical_update_cycle: 'event_driven',
    last_checked: TODAY,
  },
  {
    source_id: 'SRC_NBS_INCOME_2024',
    source_name: '国家统计局-2024年居民收入和消费支出情况',
    source_type: 'government_dataset',
    source_url: 'https://www.stats.gov.cn/sj/zxfb/202501/t20250117_1958325.html',
    credibility: 'high',
    typical_update_cycle: 'annual',
    last_checked: TODAY,
  },
...
const AUDIT_STATUS_BY_URL = new Map();
try {
  if (fs.existsSync(URL_AUDIT_PATH)) {
    const audit = JSON.parse(fs.readFileSync(URL_AUDIT_PATH, 'utf8'));
    (audit.rows || []).forEach((r) => {
      const u = normalizeUrl(r.url);
      if (u) AUDIT_STATUS_BY_URL.set(u, Number(r.http_status || 0));
    });
  }
} catch {
...
function canonicalizeSourceType(v) {
  if (!v) return 'curated_mapping';
  return SOURCE_TYPE_CANONICAL_MAP[v] || v;
}

function inferStatus(url) {
  const normalized = normalizeUrl(url);
  if (AUDIT_STATUS_BY_URL.has(normalized)) {
    return AUDIT_STATUS_BY_URL.get(normalized);
  }
...
function inferSourceType(url, fallbackType = null) {
  if (fallbackType) return canonicalizeSourceType(fallbackType);
  const host = parseHost(url);
  if (!host) return 'curated_mapping';
...
raw['来源注册表'] = raw['来源注册表'] || [];
const KEY_CITY_IDS = (raw['枚举字典']?.['关键城市'] || []).map((x) => x.city_id).filter(Boolean);
...
function registerRegistryItem(item) {
  ...
  item.source_type = canonicalizeSourceType(item.source_type);
...
function ensureSourceObject(src, fallback = {}) {
  ...
  src.source_type = canonicalizeSourceType(src.source_type);
  if (!src.source_date) {
...
function collectionPriority(collectionKey) {
  if (['笔试真题库', '面试真题库', '薪酬快照_按城市_按公司层级_按岗位'].includes(collectionKey)) return 'P0';
  if (['从业者访谈', '案例复盘', '争议问题与结论'].includes(collectionKey)) return 'P1';
  return 'P2';
}

function resolveManualRequired(entry, collectionKey, defaultRequired) {
  let required = defaultRequired;
  if (TECH_INDUSTRIES.has(entry.industry_id)) {
    if (collectionKey === '笔试真题库' || collectionKey === '面试真题库') required = Math.max(required, 12);
    if (collectionKey === '从业者访谈') required = Math.max(required, 6);
    if (collectionKey === '案例复盘') required = Math.max(required, 4);
    if (collectionKey === '争议问题与结论') required = Math.max(required, 3);
  } else if (FINANCE_INDUSTRIES.has(entry.industry_id)) {
    if (collectionKey === '笔试真题库' || collectionKey === '面试真题库') required = Math.max(required, 11);
    if (collectionKey === '从业者访谈') required = Math.max(required, 6);
    if (collectionKey === '案例复盘') required = Math.max(required, 4);
    if (collectionKey === '争议问题与结论') required = Math.max(required, 3);
  } else if (PUBLIC_INDUSTRIES.has(entry.industry_id)) {
    if (collectionKey === '笔试真题库' || collectionKey === '面试真题库') required = Math.max(required, 14);
    if (collectionKey === '从业者访谈') required = Math.max(required, 6);
    if (collectionKey === '案例复盘') required = Math.max(required, 4);
    if (collectionKey === '争议问题与结论') required = Math.max(required, 3);
  }
...
function ensureManualSlots(entry, collectionKey, cfg) {
  ...
  const required = resolveManualRequired(entry, collectionKey, cfg.required);
  const current = col.items.length;
  const pending = Math.max(0, required - current);
  const priority = collectionPriority(collectionKey);
  const dueDate = priority === 'P0' ? '2026-03-10' : (priority === 'P1' ? '2026-03-31' : '2026-04-15');
  ...
  for (let i = 0; i < pending; i += 1) {
    col.manual_fill_slots.push({
      ...
      note: '该条目需补充真实样本，未填前不计入verified。若无法联网获取，请手工填入并保留证据链接。',
      created_at: TODAY,
    });
  }
  ...
  col.collection_tasks = pending > 0 ? [
    {
      task_id: `${entry.industry_id}_${collectionKey.replace(/[^A-Za-z0-9]/g, '_')}_TASK_001`,
      priority,
      due_date: dueDate,
      owner: entry.meta?.owner || 'owner-unassigned',
      status: 'todo',
      objective: `补齐${collectionKey}到最小样本${required}条`,
...
function ensureSalarySplit(entry) {
  ...
  const existingCityIds = Array.from(new Set((micro.items || []).map((x) => x.city_id).filter(Boolean)));
  const targetCityCount = PUBLIC_INDUSTRIES.has(entry.industry_id)
    ? 5
    : ((TECH_INDUSTRIES.has(entry.industry_id) || FINANCE_INDUSTRIES.has(entry.industry_id)) ? 8 : 6);
  const targetCityIds = KEY_CITY_IDS.slice(0, targetCityCount);
  const missingCityIds = targetCityIds.filter((cid) => !existingCityIds.includes(cid)).slice(0, 3);

  const cityLevelSlots = missingCityIds.map((cid, idx) => ({
    slot_id: `${entry.industry_id}_SALARY_MICRO_CITY_FILL_${String(idx + 1).padStart(3, '0')}`,
    status: 'pending_user_fill',
    priority: 'P0',
    due_date: '2026-03-10',
    city_id: cid,
    ...
    note: '请补充该城市至少1条真实薪酬样本，优先校招主力岗位。',
    created_at: TODAY,
  }));
  ...
  const salaryRequiredCount = 2 + cityLevelSlots.length;
  micro.manual_fill_progress = {
    required_count: salaryRequiredCount,
    current_count: 0,
    pending_count: salaryRequiredCount,
...
function enrichPolicyLogs(entry) {
  const col = entry.dynamic?.['政策变化日志'];
  if (!col || typeof col !== 'object') return;
  col.items = Array.isArray(col.items) ? col.items : [];

  const existingIds = new Set(col.items.map((x) => x.log_id));
  const additions = [
    {
      log_id: `${entry.industry_id}_POLICY_20260107_MOE`,
      date: '2026-01-07',
      title: '教育部召开2026届高校毕业生就业创业工作会议',
      impact: '强化高校毕业生就业服务与岗位供给协同，影响春招与全年校招节奏。',
      scope: '全国',
      updated_at: TODAY,
      evidence: {
        source_id: 'SRC_MOE_MEETING_2026_JAN',
        source_name: '教育部-2026届高校毕业生就业创业工作会议',
        source_url: 'https://www.moe.gov.cn/jyb_xwfb/gzdt_gzdt/s5987/202601/t20260107_1425856.html',
        source_type: 'government_policy',
        source_date: '2026-01-07',
        sample_size: 1,
        stat_definition: '教育部官方发布，反映年度高校毕业生就业工作部署',
        confidence: 0.86,
      },
      authenticity_level: 'official',
      data_origin: 'structured_collection',
    },
    {
      log_id: `${entry.industry_id}_POLICY_20260129_MOHRSS`,
      date: '2026-01-29',
      title: '人社部发布春季高校毕业生就业服务活动信息',
      impact: '提升春招阶段岗位触达与跨区域流动匹配效率。',
      scope: '全国',
      updated_at: TODAY,
      evidence: {
        source_id: 'SRC_MOHRSS_CAMPUS_2026_0129',
        source_name: '人社部中国公共招聘网-春季促就业活动',
        source_url: 'https://chinajob.mohrss.gov.cn/h5/c/2026-01-29/486013.shtml',
        source_type: 'government_platform',
        source_date: '2026-01-29',
        sample_size: 1,
        stat_definition: '中国公共招聘网发布的高校毕业生就业服务活动信息',
        confidence: 0.82,
      },
      authenticity_level: 'official',
      data_origin: 'structured_collection',
    ...
    {
      log_id: `${entry.industry_id}_POLICY_20250117_NBS`,
      date: '2025-01-17',
      title: '国家统计局发布2024年居民收入和消费支出情况',
      impact: '为薪酬预期与城市生活成本判断提供宏观参照。',
      scope: '全国',
      updated_at: TODAY,
      evidence: {
        source_id: 'SRC_NBS_INCOME_2024',
        ...
        sample_size: 1,
        stat_definition: '国家统计局官方发布，作为就业与薪酬预期的宏观背景数据',
        confidence: 0.8,
      },
      authenticity_level: 'official',
      data_origin: 'structured_collection',
  ...
  additions.forEach((item) => {
    if (!existingIds.has(item.log_id)) {
      col.items.push(item);
      existingIds.add(item.log_id);
    }
  });
}
...
function setMeta(entry, previousStatus) {
  ...
  const p0Pending = Object.values(entry.dynamic || {})
    .flatMap((c) => (Array.isArray(c?.manual_fill_slots) ? c.manual_fill_slots : []))
    .filter((slot) => slot?.priority === 'P0' && String(slot?.status || 'pending').startsWith('pending')).length;

  let status = 'draft';
  let reason = '核心集合仍存在较多待人工补录槽位。';

  if (quality >= 75 && verified >= 6 && pending <= 28 && real >= 50 && p0Pending === 0) {
    status = 'published';
    reason = '质量分、已验证集合数、人工待补录规模与P0清零条件均达到发布阈值。';
  } else if (quality >= 60) {
    status = 'reviewed';
    reason = p0Pending > 0 ? '结构与证据可用，但P0槽位未清零，需持续补充真实样本。' : '结构与证据可用，但仍需持续补充真实样本。';
  }

  entry.meta.content_version = '1.5.0';
  ...
  entry.meta.state_history = [
    {
      ...
      reason: status === previousStatus
        ? 'v1.5.0二次增强（来源类型标准化、官方政策增量、人工补录任务分级）后状态保持不变。'
        : 'v1.5.0二次增强后按质量阈值重判状态。',
      changed_at: NOW_LOCAL,
      changed_by: 'system_upgrade_v1_5',
      trigger: 'scripts/upgrade_v1_5_full.js',
    },
  ];
}
...
function ensureEntrySources(entry) {
  ...
  const mustHaveIds = [
    ...
    'SRC_NCSS_2026_JOINT',
    'SRC_MOE_MEETING_2026_JAN',
    'SRC_MOHRSS_CAMPUS_2026_0129',
  ];
...
function buildDictionaries(root) {
  ...
  (root['来源注册表'] || []).forEach((r) => {
    if (r.source_type) sourceTypes.add(canonicalizeSourceType(r.source_type));
  });
  ...
  (root['行业词条'] || []).forEach((e) => {
    ...
    (e.sources || []).forEach((s) => {
      if (s.source_type) sourceTypes.add(canonicalizeSourceType(s.source_type));
    });
  });
...
function updateGovernance(root) {
  root['治理配置'] = root['治理配置'] || {};
  root['治理配置']['审核要求'] = '所有可发布记录必须具备可追溯source_id（映射到来源注册表）和可验证source_url；低可访问来源需添加人工补录槽位，并设置优先级与到期日。';
  ...
  root['治理配置']['来源类型标准映射'] = SOURCE_TYPE_CANONICAL_MAP;
  root['治理配置']['采集优先级规则'] = {
    P0: ['笔试真题库', '面试真题库', '薪酬快照_按城市_按公司层级_按岗位'],
    P1: ['从业者访谈', '案例复盘', '争议问题与结论'],
    P2: ['其他集合'],
  };
}

function updateMetadata(root) {
  root['文档元数据']['文档名称'] = '中国大陆应届生求职行业百科（v1.5任务化运营版）';
  root['文档元数据']['版本'] = 'v1.5.0';
  root['文档元数据']['发布日期'] = TODAY;
  root['文档元数据']['说明'] = [
    'v1.5.0引入URL巡检结果，刷新来源可访问性状态。',
    '统一source_type到标准枚举，降低来源类型漂移。',
    '按行业与集合分配人工补录优先级（P0/P1/P2）与到期日。',
    '补充教育部/人社部/统计局近年政策日志，并输出可直接填报模板。',
  ];

  root['文档元数据']['变更记录'] = root['文档元数据']['变更记录'] || [];
  root['文档元数据']['变更记录'] = root['文档元数据']['变更记录'].filter((x) => x.version !== 'v1.5.0');
  root['文档元数据']['变更记录'].push({
    version: 'v1.5.0',
    date: TODAY,
    summary: [
      '基于url_status_audit结果刷新来源可访问性',
      '新增教育部/人社部/统计局政策增量来源并写入政策日志',
      '人工补录槽位升级为分级任务（priority/due_date）',
      'source_type标准化并输出fill_templates_v1_5供人工补录',
    ],
  });
}

function exportFillTemplates(root) {
  fs.mkdirSync(TEMPLATE_OUT_DIR, { recursive: true });
  const index = [];

  (root['行业词条'] || []).forEach((entry) => {
    const template = {
      generated_at: NOW_LOCAL,
      version: root['文档元数据']?.['版本'] || 'v1.5.0',
      industry_id: entry.industry_id,
      industry_name: entry['行业名称'],
      owner: entry.meta?.owner || null,
      pending_total: entry.progress?.manual_fill_pending_total || 0,
      collections: [],
    };

    Object.entries(entry.dynamic || {}).forEach(([key, col]) => {
      const pending = col?.manual_fill_progress?.pending_count || 0;
      if (pending <= 0) return;
      template.collections.push({
        collection: key,
        priority: collectionPriority(key),
        due_date: collectionPriority(key) === 'P0' ? '2026-03-10' : (collectionPriority(key) === 'P1' ? '2026-03-31' : '2026-04-15'),
        required_count: col.manual_fill_progress?.required_count || 0,
        current_count: col.manual_fill_progress?.current_count || 0,
        pending_count: pending,
        slots: col.manual_fill_slots || [],
      });
    });

    const path = `${TEMPLATE_OUT_DIR}/${entry.industry_id}.fill.json`;
    fs.writeFileSync(path, `${JSON.stringify(template, null, 2)}\n`, 'utf8');
    index.push({
      industry_id: entry.industry_id,
      industry_name: entry['行业名称'],
      pending_total: template.pending_total,
      file: path,
    });
  });

  fs.writeFileSync(`${TEMPLATE_OUT_DIR}/index.json`, `${JSON.stringify({
    generated_at: NOW_LOCAL,
    version: root['文档元数据']?.['版本'] || 'v1.5.0',
    total_industries: index.length,
    items: index,
  }, null, 2)}\n`, 'utf8');
}

function exportSourceVerifyReport(root) {
  const rows = (root['来源注册表'] || [])
    .filter((x) => x.manual_verification_required === true)
    .map((x) => ({
      ...
      access_check: x.access_check,
      manual_verification_note: x.manual_verification_note || '请补充可访问替代链接或截图证据。',
    }));

  fs.writeFileSync(SOURCE_VERIFY_REPORT, `${JSON.stringify({
    generated_at: NOW_LOCAL,
    version: root['文档元数据']?.['版本'] || 'v1.5.0',
    total_unresolved_sources: rows.length,
    unresolved_sources: rows,
  }, null, 2)}\n`, 'utf8');
}
...
raw['来源注册表'] = raw['来源注册表']
  .map((x) => {
    ...
    y.source_type = canonicalizeSourceType(y.source_type);
...
exportFillTemplates(raw);
exportSourceVerifyReport(raw);

fs.writeFileSync(DATA_PATH, `${JSON.stringify(raw, null, 2)}\n`, 'utf8');
console.log(`Upgraded to v1.5.0: ${(raw['行业词条'] || []).length} entries`);
