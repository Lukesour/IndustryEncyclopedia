...
## 本次更新要点（2026-02-19）
- 主数据升级到 `v1.49.0`，继续执行题库同权策略（official_original / real_recall / jd_mapping）。
- 按 `v1.48.0` 任务板完成第1批直写：`IND_INTERNET_AI` 与 `IND_CIVIL_SERVICE` 各新增2个细分岗位，并逐岗位新增分阶段题目（提前批/主批/补录/实习转正）。
- 新增岗位保留平台受限补录位（BOSS/小红书检索词、必填字段、截图规则），便于后续补录不可稳定抓取信息。
