# 中国大陆应届生求职行业百科

本仓库包含针对中国大陆应届生求职市场的行业百科数据与校验脚本。

## 主要文件
- `行业百科.json`：主数据文件（当前版本 `v1.58.0`）
- `行业百科.schema.json`：JSON Schema
- `reports/`：质量报告与来源核验报告
- `scripts/`：校验与质量流水线脚本
- `data/`：辅助数据文件
- `docs/信息源分级与抓取规范_v1.32.0.md`：一级/二级证据与抓取字段规范（含最新官方链接与Cookie说明）
- `docs/题库同权与扩容规则_v1.45.0.md`：题库同权规则、扩容目标与去模板化门槛
- `docs/23行业细分岗位与题库扩展包_v1.46.0.md`：23行业逐行业扩展内容（细分岗位、笔面试题、平台补录位）
- `docs/评分口径与扣分明细_v1.32.0.md`：行业质量分可解释字段与扣分项定义
- `docs/应届生决策卡模板_v1.28.0.md`：行业横向比较的三卡模板（去哪投/何时投/值不值投）
- `docs/平台样本补录指引_v1.42.1.md`：BOSS/小红书等平台样本补录字段、检索词与质检规范
- `docs/平台可达性与补录执行_v1.55.0.md`：v1.55平台可达性实测结果、补录顺序与缺口位处理规则
- `docs/核心岗4阶段覆盖与岗位深链率专项_v1.57.0.md`：v1.57下一批次专项执行规则（核心岗4阶段+岗位深链率）
- `docs/逐岗补录与体验门槛_v1.58.0.md`：v1.58逐岗补录规则与双层门槛（发布/体验）说明
- `docs/百科深度升级执行规范_v1.60.0.md`：v1.60岗位扩容、题库分层、平台受限留空与来源分层执行规范
- `reports/全量扩容执行清单_v1.45.0.md`：23行业逐行业扩容清单（岗位、题库、缺口与检索入口）
- `reports/全量扩容执行清单_v1.45.0.json`：结构化扩容清单，可直接分配执行
- `reports/23行业信息缺口与检索卡_v1.47.0.md`：23行业逐行业平台可达性、缺口字段与补录步骤
- `reports/23行业信息缺口与检索卡_v1.47.0.json`：结构化检索卡，含可达性与替代来源
- `reports/23行业逐个填写任务板_v1.48.0.md`：逐行业岗位/题库缺口、检索词、来源与回填规则
- `reports/23行业逐个填写任务板_v1.53.0.md`：v1.53深度硬门禁口径下的逐行业回填任务板（现14岗->目标24岗）
- `reports/23行业信息缺口与检索卡_v1.53.0.md`：按v1.53重算的23行业缺口与平台检索卡（含受限回填规则）
- `reports/23行业信息缺口与检索卡_v1.53.0.json`：上述检索卡的结构化版本
- `reports/23行业逐个填写任务板_v1.54.0.md`：v1.54行业差异化门禁口径下的逐行业回填任务板
- `reports/23行业信息缺口与检索卡_v1.54.0.md`：按v1.54重算的23行业缺口与检索卡
- `reports/23行业分行业扩容执行清单_v1.60.0.md`：v1.60分行业扩容优先清单（P0/P1/P2，岗位包+笔试包+面试包）
- `reports/23行业分行业扩容执行清单_v1.60.0.json`：上述清单的结构化版本（便于任务分发与跟踪）
- `reports/信息源可达性快照_v1.60.0_20260219.md`：v1.60信息源连通性实测与平台受限回填规则
- `reports/depth_gap_latest.json`：v1.55百科深度门槛报告（高增长岗落地率、核心岗4阶段、岗位深链、平台待验证占比）
- `reports/core4_deeplink_special_latest.json`：v1.57下一批次专项缺口报告（核心岗4阶段覆盖+岗位深链率）
- `reports/core4_deeplink_special_latest.md`：v1.57专项可执行任务卡（含待补字段、检索方式、检索入口）

## 本次更新要点（2026-02-19）
- 主数据升级到 `v1.58.0`，清零题库-role画像映射断层（新增189个细分岗位画像）。
- 岗位画像新增三元证据链（官方+平台+回忆）与平台受限待补位（需要信息、检索词、入口、留证规则）。
- 治理配置新增“发布硬门槛 + 体验门槛”双层机制，支持行业差异化岗位规模目标。
- 质量流水线新增v1.58叠加指标：映射完整性、深链质量、题干去重率、决策模块厚度。
- 新增 v1.60 执行文件：23行业分行业扩容优先清单（P0/P1/P2）与结构化任务包（岗位包+笔试包+面试包）。
- 新增 v1.60 联网快照：官方/企业/平台入口可达性核验，以及 BOSS/小红书受限时的留空字段与检索回填规则。

## 校验命令
```bash
bash scripts/validate_industry_encyclopedia.sh 行业百科.schema.json 行业百科.json
bash scripts/validate_industry_references.sh 行业百科.json
node scripts/recompute_progress_v1_15.js 行业百科.json
bash scripts/run_quality_pipeline.sh 行业百科.json
node scripts/rebuild_main_from_entries_v1_58.js
node scripts/rebuild_main_from_entries_v1_57.js
bash scripts/split_industry_files.sh 行业百科.json data
```

## 前端审阅页
已新增静态审阅页：`review-ui/`，用于逐行业查看深度指南、决策卡、岗位画像、题库与来源证据，帮助快速判断需要增删改的信息。

启动方式：
```bash
python3 -m http.server 8000
```

浏览器打开：
`http://localhost:8000/review-ui/`

页面能力：
- 行业搜索与“仅看高风险行业”过滤
- 审阅信号看板（深度长度、回忆题占比、来源首页占比、岗位画像差异度）
- 深度指南、决策卡、题库、来源全量展开查看
- 自动加载失败时支持手动上传 `行业百科.json`
