...
## 本次更新要点（2026-02-19）
- 主数据升级到 `v1.52.0`，继续执行题库同权策略（official_original / real_recall / jd_mapping）。
- 按 `v1.48.0` 任务板完成第4批直写：新增5个行业（高端制造与工业自动化、汽车与智能驾驶、快消与零售、金融-保险、化工与新材料）扩容。
- 上述5行业均新增2个细分岗位，并逐岗位新增分阶段笔试/面试题（提前批/主批/补录/实习转正）。
- 新增岗位统一保留平台受限补录位（BOSS/小红书检索词、必填字段、截图规则），便于后续补录不可稳定抓取信息。
- 叠加前两批后，23行业任务板已全部完成当前轮次扩容（逐行业新增细分岗位与阶段化题库）。
