...
## 本次更新要点（2026-02-19）
- 主数据升级到 `v1.53.0`，继续执行题库同权策略（official_original / real_recall / jd_mapping）。
- 发布硬门槛升级为“深度硬门禁”：岗位规模、每岗题量、四阶段覆盖、岗位深链接、模板复用全部进入阻断。
- `scripts/recompute_progress_v1_15.js` 与治理目标对齐：移除旧的5岗/18题封顶逻辑，按配置目标重算深度评分。
- `scripts/run_quality_pipeline.sh` 新增 `template_reuse_v145` 指标，并在 `quality_gate` 输出深度缺口与模板复用超限记录。
- 主文件与 `data/entries` 已重新分拆同步，修复版本与岗位规模口径漂移问题。
