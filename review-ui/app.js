...
function textLength(value, ignoreUrls = false) {
  ...
  if (Array.isArray(value)) return value.reduce((s, x) => s + textLength(x, ignoreUrls), 0);
  if (value && typeof value === "object") return Object.values(value).reduce((s, x) => s + textLength(x, ignoreUrls), 0);
  return 0;
}
...
function buildEntryMetrics(entry) {
  const guide = safeGet(entry, ["static", "决策输出", "深度求职指南_v1_44_2026Q1"], {});
  const guideCoreChars = textLength(
    {
      ...
      决策闭环四问: guide["决策闭环四问_v1_45_2026Q1"],
      分人群投递策略: guide["分人群投递策略_v1_45_2026Q1"],
    },
    true
  );
  ...
  const whereApply = safeGet(entry, ["static", "决策输出", "decision_cards", "where_to_apply"], {});
  const roleCount = (whereApply["主投岗位"] || []).length;
  ...
  const m = {
    ...
    roleVariation,
    decisionShape: `${roleCount}/${cityCount}/A${aCount}/B${bCount}`,
  };
...
function setMetaLine() {
  ...
  const version = meta["版本"] || safeGet(state.raw, ["文档元数据", "当前版本"]) || safeGet(state.raw, ["文档元数据", "schema_ref"], "");
  const release = meta["发布日期"] || "-";
  el.metaLine.textContent = `共 ${state.entries.length} 个行业 | 发布日期 ${release} | ${version || "v1.44.x"}`;
}
...
function entryMatches(entry, q) {
  ...
  const text = JSON.stringify(
    {
      name: entry["行业名称"],
      guide: safeGet(entry, ["static", "决策输出", "深度求职指南_v1_44_2026Q1"], {}),
...
function renderHeadline(entry, m) {
  el.headlineCard.innerHTML = `
    <h2>${entry["行业名称"]}</h2>
    <p>行业ID：<span class="mono">${entry.industry_id}</span> | 状态：${safeGet(entry, ["meta", "status"], "-")} | 质量分：${m.quality.toFixed(1)}</p>
    <div class="metrics-grid">
      ...
      <div class="metric"><div class="k">权威信息源</div><div class="v">${m.guideSourcesCount}</div></div>
...
function renderSignals(entry, m) {
  ...
  el.signalsCard.innerHTML = `
    ...
    <div class="metrics-grid">
      ...
      <div class="metric"><div class="k">来源人工核验</div><div class="v">${(entry.sources || []).filter((s) => s.manual_verification_required).length}</div></div>
    </div>
  `;
}
...
function renderDecision(entry) {
  ...
  const matrix = decision["分人群投递矩阵_v1_45_2026Q1"] || safeGet(entry, ["static", "决策输出", "深度求职指南_v1_44_2026Q1", "分人群投递策略_v1_45_2026Q1"], []);
    ...
    <div class="table-wrap">
      <table>
        ...
        <tbody>
          ...
          <tr><td>门禁提示</td><td>${decision["决策门禁提示_v1_45_2026Q1"] || "-"}</td></tr>
        </tbody>
      </table>
    </div>
    <h4>分人群投递策略</h4>
...
function renderRoles(entry) {
  ...
  el.rolesCard.innerHTML = `
    ...
    <div class="table-wrap">
      <table>
        <thead><tr><th>岗位</th><th>入行门槛</th><th>转岗提示</th></tr></thead>
        <tbody>${rows || `<tr><td colspan="3">暂无岗位画像</td></tr>`}</tbody>
      </table>
    </div>
  `;
}
...
function renderQuestions(entry) {
  ...
  const writtenHtml = written.map(renderOne).join("");
  const interviewHtml = interview.map(renderOne).join("");
...
function afterLoad(raw) {
  ...
  if (!state.selectedId && state.entries.length) state.selectedId = state.entries[0].industry_id;
  setMetaLine();
  render();
}
